...
for j in range(4):
    if j < 2:
        plt.subplot(4,2,j+1)
        plt.title(names[j%2])
        plt.hist(sqsu[j%2], bins=bins, density=True)
        plt.subplot(4,2,j+3)
        plt.hist(sqs[j%2], bins=bins, density=True)
    else:
        plt.subplot(4,2,j+3)
        plt.title(names[j%2])
        plt.hist(rqsu[j%2*3], bins=bins, density=True)
        plt.subplot(4,2,j+5)
        plt.hist(rqs[j%2*3], bins=bins, density=True)
    if j < 2:
