...
import sys
sys.path.append('../')
from calibration.extraction import extract_distribution, kolmogrov_smirnov_test
from calibration.rmatching import convolved_circle_px
...
for z, sig in enumerate(sigmas):
    for i, r in enumerate(r_test):
        for j, dist in enumerate(distributions):
            bins, model = convolved_circle_px(r, h_dist[j], sig, 30)
